...
import { FaLocationDot, FaPhone, FaEnvelope, FaChevronDown, FaDiamondTurnRight } from 'react-icons/fa6';
...
export default function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: "", email: "", phone: "", subject: "", checkIn: "", checkOut: "", message: "" });
    } catch (error) {
      console.error(error);
  ...
  return (
    ...
    <label className="contact-form-label">NAME<span className="contact-required">*</span></label>
    <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Enter your name" className="contact-form-input" required />
    </div>

    <div className="contact-form-group">
    <label className="contact-form-label">EMAIL<span className="contact-required">*</span></label>
    <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Enter your email ID" className="contact-form-input" required />
    </div>

    <div className="contact-form-group">
    <label className="contact-form-label">PHONE NUMBER<span className="contact-required">*</span></label>
    ...
    <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="Enter your number" className="contact-form-input contact-phone-input" required />
    </div>
    </div>

    <div className="contact-form-group">
    <label className="contact-form-label">SUBJECT<span className="contact-required">*</span></label>
    <input type="text" name="subject" value={formData.subject} onChange={handleChange} placeholder="Enter subject" className="contact-form-input" required />
    </div>

    <div className="contact-form-group">
    <label className="contact-form-label">MESSAGE<span className="contact-required">*</span></label>
    <textarea name="message" value={formData.message} onChange={handleChange} placeholder="Enter messages or queries" rows="5" className="contact-form-textarea" required />
    </div>
    ...
    {faqData.map((faq, index) => (
      ...
      <div className="contact-faq-question" onClick={() => toggleFAQ(index)}>
      <h3>{faq.question}</h3>
    ...
    <iframe
    title="Ocean Paradise Hotel location map — 773 Ocean Shores Blvd NW, Ocean Shores WA 98569"
